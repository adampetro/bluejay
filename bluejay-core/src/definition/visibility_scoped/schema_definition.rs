...
{
    pub fn new(cache: &'a Cache<'a, S, W>) -> Self {
        let inner = cache.schema_definition();
        Self {
            inner,
            cache,
    ...
    definition::SchemaDefinition for SchemaDefinition<'a, S, W>
{
    ...
    fn get_type_definition(
        ...
        self.cache
            .get_or_create_type_definition_by_name(name)
            .map(definition::TypeDefinition::as_ref)
    }
        ...
        self.interface_implementors
            ...
            .unwrap_or_else(|| {
                let interface_implementors = self
                    ...
                    .map(|otd| {
                        // TODO: check that interface implementation is visible for object
                        self.cache
    ...
    fn get_directive_definition(&self, name: &str) -> Option<&Self::DirectiveDefinition> {
        self.cache.get_or_create_directive_definition_by_name(name)
    }
}
